...
void main() {
  ...
  const premiumProduct = BillingProduct(
  ...
  testWidgets('Successful purchase flow shows success snackbar', (tester) async {
    setSurfaceSize(tester);
    final completer = Completer<PurchaseResult>();
    when(() => mockBillingService.getProducts()).thenAnswer((_) async => [premiumProduct]);
    when(() => mockBillingService.purchase(any())).thenAnswer((_) => completer.future);
    ...
    completer.complete(const PurchaseResult(status: PurchaseStatus.success));
    await tester.pumpAndSettle();

    expect(find.text('Successfully upgraded to Premium!'), findsOneWidget);
  });

  testWidgets('Failed purchase flow shows error snackbar', (tester) async {
    setSurfaceSize(tester);
    when(() => mockBillingService.getProducts()).thenAnswer((_) async => [premiumProduct]);
    when(() => mockBillingService.purchase(any()))
        .thenAnswer((_) async => const PurchaseResult(status: PurchaseStatus.error, errorMessage: 'Payment failed'));
  ...
  testWidgets('Restore flow shows success snackbar', (tester) async {
    setSurfaceSize(tester);
    when(() => mockBillingService.getProducts()).thenAnswer((_) async => [premiumProduct]);
  ...
  testWidgets('Cancelled purchase flow does not show success', (tester) async {
    setSurfaceSize(tester);
    when(() => mockBillingService.getProducts()).thenAnswer((_) async => [premiumProduct]);
    when(() => mockBillingService.purchase(any()))
        .thenAnswer((_) async => const PurchaseResult(status: PurchaseStatus.cancelled));
    ...
    expect(find.text('Successfully upgraded to Premium!'), findsNothing);
  });
}
